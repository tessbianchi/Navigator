...
class ColorTracker{
public:
  void track(cv::Mat frame_left, std::vector<cv::Point2f> points, double image_proc_scale);
  void clear();
...
private:
  ...
  std::vector<std::vector<double>> colors_and_distance;

  cv::Scalar* last_color = NULL;
  ros::NodeHandle nh;
